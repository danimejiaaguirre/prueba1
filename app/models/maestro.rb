class Maestro < ActiveRecord::Base
  ...
  attr_accessible :cedula, :direccion, :email, :fch_ncto, :materia_id, :nombre, :telefono
  
  #Metodo de Busqueda
  def self.search(search) 	
		where("nombre like '%#{search}%' or email like '%#{search}%' or telefono like '%#{search}%' or direccion like '%#{search}%'") 
  end
